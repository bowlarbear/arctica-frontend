                ...
                for(let i = 0; i < resArray.length; i ++){
                    ...
                    if (String(it[0]).toUpperCase() == 'TYPE' && String(it[1]).toUpperCase() == 'SETUPCD'){
                        ...
                        invoke('create_backup', {number: "2"}).then((res) => {
                            ...
                            //note: not sending user to error screen here as there is an unexpected error that often occurs here that seems innocuous
                        })
                        break
                    }
                }
            }).catch((e)=> {
            store.commit('setDebug', `error reading setup CD Setup27b-8: ${e}`)
            this.noCD = true
            this.loading = false
        })
        },
