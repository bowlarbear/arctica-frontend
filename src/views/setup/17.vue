<template>
  ...
  <div v-else class="page">
      <header>
        ...
        <h2 v-if="badCD == false">Please power off this machine, remove HW 4, insert HW 5, and power on the machine.</h2>
  ...
  export default {
    ...
    methods: {
      tryAgain(){
        this.loading = true
        this.badCD = false
        //refresh the currently inserted disc with the latest CDROM iso
            ...
            invoke('stop_bitcoind').then((res)=>{
                ...
                invoke('async_write', {name: 'setupStep', value: this.setupStep}).then((res) => {
                    ...
                    this.loading = false
                        ...
                        store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: Setup17-12 Response: ${e}`)
                        this.$router.push({ name:'Error' })
                })
                }).catch((e) => {
                    store.commit('setDebug', `async write error: ${e}`)
                    store.commit('setErrorMessage', `Error with async write Error code: Setup17-9 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
            }).catch((e) => {
                    store.commit('setDebug', `packup error: ${e}`)
                    store.commit('setErrorMessage', `Error packing up Error code: Setup17-10 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
        }).catch((e)=>{
            store.commit('setDebug', `refresh setup CD error Error code: Setup17-11 Response: ${e}`)
            this.loading = false
            this.badCD = true
            })
      }
                            ...
                            invoke('install_hw_deps', {warm: false}).then((res) => {
                                    ...
                                    store.commit('setLoadMessage', 'Refreshing setup CD...')
                                        ...
                                        invoke('refresh_cd', {psbt: false}).then((res)=>{
                                            ...
                                            store.commit('setDebug', `refresh setup CD error Error code: Setup17-3 Response: ${e}`)
                                            this.loading = false
                                            this.badCD = true
                                            })
                                }).catch((e) => {
