<template>
    ...
    <div v-else class="page">
        ...
        <h2 v-if="badCD == false">Please remove HW 3 and remove the set up CD.</h2>
        <h2 v-if="badCD == false">If you are using an optional secondary machine, please switch to it now, power it off, insert HW 4 and power on the machine.</h2>
            ...
            invoke('stop_bitcoind').then((res)=>{
                ...
                invoke('async_write', {name: 'setupStep', value: this.setupStep}).then((res) => {
                    ...
                    this.loading = false
                        ...
                        store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: Setup15b-9 Response: ${e}`)
                        this.$router.push({ name:'Error' })
                    })
                }).catch((e) => {
                    store.commit('setDebug', `async write error: ${e}`)
                    store.commit('setErrorMessage', `Error with async write Error code: Setup15b-10 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
            }).catch((e) => {
                    store.commit('setDebug', `packup error: ${e}`)
                    store.commit('setErrorMessage', `Error packing up Error code: Setup15b-11 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
        }).catch((e)=>{
            store.commit('setDebug', `refresh setup CD error Error code: Setup15b-12 Response: ${e}`)
            this.loading = false
            this.badCD = true
            })
      }
                            ...
                            invoke('install_hw_deps', {warm: false}).then((res) => {
                                    ...
                                    store.commit('setLoadMessage', 'Refreshing setup CD...')
                                        ...
                                        invoke('refresh_cd', {psbt: false}).then((res)=>{
                                            ...
                                            store.commit('setDebug', `refresh setup CD error Error code: Setup15b-3 Response: ${e}`)
                                            this.loading = false
                                            this.badCD = true
                                            })
                                }).catch((e) => {
