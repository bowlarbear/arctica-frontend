  ...
  components: {
    ...
    methods: {
        acknowledge(){
            ...
            store.commit('setDebug', `error reading setup CD Setup31b-8: ${e}`)
            this.noCD = true
            this.loading = false
        })
        },
        ...
        invoke('copy_cd_to_ramdisk').then((res) => {
            ...
            invoke('unpack').then((res) => {
                ...
                invoke('create_descriptor', {hwnumber: this.hwNumber.toString()}).then((res) => {
                    ...
                    store.commit('setErrorMessage', `Error creating descriptors Error code: Setup31b-6 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                    })
            }).catch((e) => {
                store.commit('setDebug', `error unpacking sensitive info: ${e}`)
                store.commit('setErrorMessage', `Error unpacking Error code: Setup31b-5 Response: ${e}`)
                this.$router.push({ name:'Error' })
                })
        }).catch((e) => {
